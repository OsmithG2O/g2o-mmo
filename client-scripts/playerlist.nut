...
class PlayerList
{
	constructor()
	{
		...
		m_DrawName = createDraw("Èìÿ", "FONT_OLD_10_WHITE_HI.TGA", LIST.x + LIST.offset + 300, LIST.y + LIST.offset, 255, 255, 0);
		m_DrawPing = createDraw("Ïèíã", "FONT_OLD_10_WHITE_HI.TGA", LIST.x + LIST.offset + LIST.width - 450, LIST.y + LIST.offset, 255, 255, 0);
	...
	function updateMyName(pid, name)
	{
		foreach (player in m_List)
			if (player.m_Id == pid)
...
addEvent("onChangeName", function(pid, name)
{
	playerList.updateMyName(pid, name);
});
